...
//! - **Per-category counters and ahead/behind.** ShellOut only fills
//!   `branch` + `dirty` (see `parse_porcelain_v1` in
//!   `crates/p10k-rs-git/src/lib.rs`). It leaves `staged`,
//!   `unstaged`, `untracked`, `has_conflicts`, `ahead`, and `behind`
//!   at their defaults. Cross-check is therefore limited to
//!   `branch` (on branch-attached cases) and `dirty`.
...
/// Assert that both backends agree on the fields that ShellOut
/// populates: `branch` (when `expect_branch` is supplied) and
/// `dirty` (always).
///
/// `expect_branch` is `None` for the detached-HEAD case where the
/// two backends are known to disagree (see the module doccomment).
fn assert_agree(slug: &str, path: &Path, expect_dirty: bool, expect_branch: Option<&str>) {
...
fn agree_clean_repo() {
    ...
    assert_agree("clean", &scratch, false, Some("main"));
...
fn agree_untracked_file() {
    ...
    assert_agree("untracked", &scratch, true, Some("main"));
    let _ = std::fs::remove_dir_all(&scratch);
}
...
fn agree_modified_tracked_file() {
    ...
    assert_agree("modified", &scratch, true, Some("main"));
    let _ = std::fs::remove_dir_all(&scratch);
}
...
fn agree_staged_file() {
    ...
    assert_agree("staged", &scratch, true, Some("main"));
    let _ = std::fs::remove_dir_all(&scratch);
}
...
fn agree_dirty_only_on_detached_head() {
    ...
    assert_agree("detached", &scratch, false, None);
    let _ = std::fs::remove_dir_all(&scratch);
}
...
fn agree_clean_with_upstream_set() {
    ...
    assert_agree("upstream", &scratch, false, Some("main"));
