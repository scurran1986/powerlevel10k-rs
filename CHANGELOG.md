...
posture, no breaking config changes — this minor bump signals
the start of the v0.2 era rather than a deliberate breakage.
Pre-1.0 minor bumps "may be" breaking; v0.2.0 deliberately
isn't, to keep upgrade friction zero on the cycle-opener tag.

The substantive surface change is the new `p10k-rs version`
subcommand — a deeper-than-`--version` diagnostic that prints
binary version, bundled gitstatusd pin (`v1.5.4`), detected
host triple, and the binary sha256 fingerprint for the
gitstatusd at the install prefix. Useful for bug reports and
install-script diagnostics.

`--version` (auto-generated by clap from the workspace version
field) still works for the bare version string; the new
subcommand exists for the "what is this binary actually shipping
with?" question that's been answered by hand-running `verify`
+ inspecting pins.toml until now.

### `p10k-rs version`

Multi-line plain text output, always exits 0. Shape:
...
Triple line variants when applicable:

- `gitstatusd triple: <host> (no pin entry)` — supported triple
  but pin not yet recorded
- `gitstatusd triple: <unsupported host>` — `detect_host_triple()`
  didn't find a match

If the embedded pins file fails to parse the gitstatusd line
collapses to `gitstatusd: <unparseable: …>` rather than the
subcommand erroring — diagnostics should never themselves fail.
